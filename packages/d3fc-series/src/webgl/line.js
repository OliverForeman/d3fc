    ...
    const line = (data) => {
        ...
        if (!isIdentityScale(xScale.scale) || !isIdentityScale(yScale.scale) || !equals(previousData, data)) {
            ...
            if (base.orient() === 'vertical') {
                crossPreviousValueAttribute.value((data, i) => xScale.scale(base.crossValue()(data[i], i))).data(data);
                mainPreviousValueAttribute.value((data, i) => yScale.scale(base.mainValue()(data[i], i))).data(data);
            } else {
                crossPreviousValueAttribute.value((data, i) => xScale.scale(base.mainValue()(data[i], i))).data(data);
                mainPreviousValueAttribute.value((data, i) => yScale.scale(base.crossValue()(data[i], i))).data(data);
            }
