    ...
    const point = (data) => {
        ...
        if (!isIdentityScale(xScale.scale) || !isIdentityScale(yScale.scale) || !equals(previousData, data)) {
            ...
            if (base.orient() === 'vertical') {
                ...
                mainValueAttribute.value((data, i) => yScale.scale(base.mainValue()(data[i], i))).data(data);
            } else {
                crossValueAttribute.value((data, i) => xScale.scale(base.mainValue()(data[i], i))).data(data);
                mainValueAttribute.value((data, i) => yScale.scale(base.crossValue()(data[i], i))).data(data);
            }
            sizeAttribute.value((data, i) => size(data[i], i)).data(data);
